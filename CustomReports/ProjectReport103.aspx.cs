...
public partial class ProjectReport103 : Report
{
    private static int MyReportID = 103;
    protected string selProjectOptions103 = "";
    public string htmlGlobalReportTable103;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            selProjectOptions103 = BindChoices(dataSet.Tables["DropDownFieldsChoice"]);  // 绑定下拉框字段值            

            dataSet = db.GetGlobalReportsInfo(MyReportID, hidprojectIDs.Value.ToString());
            htmlGlobalReportTable103 = GetReportByHTML(dataSet.Tables["ReportTable103"]);
        }
    }
    ...
    [WebMethod]
    public static string RefreshGlobalReport103(string selectedProjectsStr)
    {
        DBL db = new DBL();
        DataSet dataSet;

        dataSet = db.GetGlobalReportsInfo(MyReportID, selectedProjectsStr);
        return GetReportByHTML(dataSet.Tables["ReportTable103"]);
    }
    ...
    public void exportReportToExcel(Object sender, EventArgs e)
    {
        ...
        {
            fullHtml.Append(ReportHtmlStart(4, "测试报表3"));
    ...
    public static string getReportTableString(string multiSelectFieldValue)
    {
        ...
        DataTable dt = dataSet.Tables["ReportTable103"];
