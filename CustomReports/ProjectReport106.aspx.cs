...
public partial class ProjectReport106 : Report
{
    private static int MyReportID = 106;
    protected string selProjectOptions106 = "";
    public string htmlGlobalReportTable106;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            selProjectOptions106 = BindChoices(dataSet.Tables["DropDownFieldsChoice"]);

            string strFlag = string.Format("{0}_{1}_{2}", startDate, endDate, hidprojectIDs.Value.ToString());
            dataSet = db.GetGlobalReportsInfo(MyReportID, strFlag);
            htmlGlobalReportTable106 = GetReportByHTML(dataSet.Tables["ReportTable106"]);
        }
    }


    [WebMethod]
    public static string RefreshGlobalReport106(string startDateStr, string endDateStr, string selectedProjectsStr)
    {
        DBL db = new DBL();
        DataSet dataSet;
        ...
        dataSet = db.GetGlobalReportsInfo(MyReportID, strFlag);
        return GetReportByHTML(dataSet.Tables["ReportTable106"]);
    }
    ...
    private string BindChoices(DataTable dt)
    {
        ...
        hidprojectIDs.Value = dt.Rows[0]["ProjectID"].ToString();
        return sb.ToString();
    }
    ...
    public void exportReportToExcel(Object sender, EventArgs e)
    {
        ...
        {
            fullHtml.Append(ReportHtmlStart(4, "测试报表6")); 
    ...
    public static string getReportTableString(DateTime startDate, DateTime endDate, string selectedProjectsStr)
    {
        ...
        string strFlag = string.Format("{0}_{1}_{2}", startDate, endDate, selectedProjectsStr);
        dataSet = db.GetGlobalReportsInfo(MyReportID, strFlag);
        DataTable dt = dataSet.Tables["ReportTable106"];
